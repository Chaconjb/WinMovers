...
namespace WinMovers.Controllers
{
    public class OrdenTrabajoController : Controller
    {
        ...
        // Obtiene el Id (int) del usuario autenticado actual, o null si no hay sesi�n.
        ...
        public async Task<IActionResult> Edit(int id, OrdenTrabajo orden)
        {
            ...
            // Registrar auditor�a si cambia fecha_servicio o estado
        ...
        public async Task<IActionResult> SubirArchivo(int idOrden, IFormFile archivo)
        {
            ...
            {
                return Json(new { ok = false, mensaje = "El archivo supera el l�mite de 10 MB permitido." });
            }

            if (!TiposPermitidos.Contains(archivo.ContentType))
            {
                return Json(new { ok = false, mensaje = "Solo se permiten PDF o im�genes (JPG, PNG, WEBP)." });
            }
        ...
        // La vista Materiales.cshtml ya publicaba a esta acción, que no existía.
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> EliminarMaterial(int idOrdenMaterial)
        {
            ...
            if (asignacion == null)
            {
                TempData["Error"] = "La asignación de material ya no existe.";
                return RedirectToAction(nameof(Index));
            }

            var idOrden = asignacion.IdOrden;

            // Devolver al inventario lo que AsignarMaterial había descontado.
        ...
        public async Task<IActionResult> AgregarNota(int idOrden, string contenido)
        {
            ...
            {
                TempData["Error"] = "La nota no puede estar vac�a.";
